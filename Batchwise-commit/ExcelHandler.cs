...
namespace ORSResearchTool
{
    public static class ExcelHandler
    {
        ...
        // ── Checkpoint: append one record (kept for WriteResults compatibility) ────────

        /// <summary>
        /// Appends a single record. Used internally by WriteResults only.
        /// For live processing use AppendBatch instead.
        /// </summary>
        public static void AppendResult(string outputPath, OrsRecord record)
        {
            AppendBatch(outputPath, new List<OrsRecord> { record });
        }

        // ── Checkpoint: append a batch of records in one open/save cycle ──────────────

        /// <summary>
        /// Opens the output file once, appends every record in
        /// <paramref name="batch"/>, then saves once.
        /// Called by OrsResearcher.FlushBuffer every 200 records (and on final flush).
        /// One file open per 200 records instead of one per record.
        /// Thread-safe via a per-file lock.
        /// </summary>
        public static void AppendBatch(string outputPath, IList<OrsRecord> batch)
        {
            ...
            lock (GetFileLock(outputPath))
            {
                using (var package = new ExcelPackage(new FileInfo(outputPath)))
                {
                    ...
                    // Safety: recreate header if sheet is somehow empty
                    if (ws.Dimension == null)
                        WriteHeaderRow(ws);
                    ...
                    package.Save();   // single save for the entire batch
                }
            }
        }
